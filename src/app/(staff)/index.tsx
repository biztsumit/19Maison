...
function formatPrice(item: HomepageProductItem): string {
  const amount = item.discount > 0 ? item.discountedPrice : item.price;
...
function SectionHeading({
  ...
  return (
    <View style={styles.sectionHeadingRow}>
      ...
      <TouchableOpacity
        activeOpacity={0.8}
        onPress={() => router.push('/(staff)/category')}
      >
        <Text style={[styles.viewMore, { fontSize: viewMoreSize }]}>
          View more
        </Text>
      </TouchableOpacity>
    </View>
...
export default function StaffHomeScreen() {
  ...
  const { data: collectors, isLoading: collectorsLoading } =
    useCollectorsEditionDrop();
